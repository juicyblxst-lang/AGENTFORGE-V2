...
async def prepare(h:Hire):
    try:
        ...
        # The client signs createJob; the API returns only validated orchestration inputs.
...
async def budget(job_id:int):
    try:
        ...
        upsert(job_id,status='budgeted',create_tx=None)
        return {'ok':True,'txHash':tx}
    except Exception as e: raise HTTPException(400,str(e))

@app.post('/api/jobs/{job_id}/record')
async def record(job_id:int,h:Hire):
    upsert(job_id,agent_registry=h.agent_registry,agent_id=h.agent_id,client=h.client,status='created')
    return {'ok':True}
