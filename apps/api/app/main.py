...
class TxRecord(BaseModel):
    agent_id:str|None=None; agent_registry:str|None=None; client:str|None=None; description:str|None=None
    create_tx:str|None=None; register_tx:str|None=None; budget_tx:str|None=None; fund_tx:str|None=None
...
@app.get('/health')
async def health(): return {'ok':True,'network':'BSC Testnet','chainId':97,'providerConfigured':bool(settings.provider_private_key and settings.provider_address and settings.provider_agent_base_url)}
@app.get('/api/config')
...
async def record(job_id:int,h:TxRecord):
    ...
    upsert(job_id,**fields,status='created'); return {'ok':True}
...
# Public deliverable endpoint required by bnbagent LocalStorageProvider.
# The SDK publishes {ERC8183_AGENT_URL}/job/{id}/response when local storage is used.
