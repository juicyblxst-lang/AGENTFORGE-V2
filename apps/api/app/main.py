...
if vercel_url not in allowed_origins: allowed_origins.append(vercel_url)
...
def _keep_alive():
    while True:
        time.sleep(240)
        try: urllib.request.urlopen('http://localhost:10000/health', timeout=5)
...
def _run_worker_thread():
    ...
    try: worker_loop.run_until_complete(worker())
    except asyncio.CancelledError: pass
    except Exception: logger.exception('Provider worker stopped unexpectedly')
    finally: worker_loop.close()
...
async def shutdown():
    global worker_loop
    if worker_loop and worker_loop.is_running(): worker_loop.call_soon_threadsafe(worker_loop.stop)
...
async def track_metadata(track: str):
    if track not in TRACK_META: raise HTTPException(404, f'Unknown track: {track}')
    return TRACK_META[track]

@app.post('/agent/rebalancing')
async def agent_rebalancing(payload: dict): return {"result": f"Rebalancing executed for task: {payload.get('task', 'no task')}"}
@app.post('/agent/grid-trading')
async def agent_grid_trading(payload: dict): return {"result": f"Grid trading executed for task: {payload.get('task', 'no task')}"}
@app.post('/agent/yield-optimisation')
async def agent_yield_optimisation(payload: dict): return {"result": f"Yield optimisation executed for task: {payload.get('task', 'no task')}"}
@app.post('/agent/health-factor-monitoring')
async def agent_health_factor_monitoring(payload: dict): return {"result": f"Health factor monitoring executed for task: {payload.get('task', 'no task')}"}
...
def _find_hardcoded(agent_id: str, agent_registry: str):
    for a in HARDCODED_AGENTS:
        if a['agentId'] == agent_id and a['agentRegistry'].lower() == agent_registry.lower(): return a
    for a in HARDCODED_AGENTS:
        if a['agentId'] == agent_id: return a
    return None
...
async def agents(limit: int = 100):
    try:
        ...
        if discovered: return {'agents': discovered}
    except Exception as e: logger.error(f"Discovery failed: {e}")
...
async def prepare(h: Hire):
    try:
        ...
        if not agent: agent = await get_agent(int(h.agent_id))
...
async def record(job_id: int, h: TxRecord):
    fields = h.model_dump(exclude_none=True); fields.pop('description', None); upsert(job_id, **fields); return {'ok': True}
...
@app.get('/api/jobs')
async def list_jobs(limit: int = 50): return {'jobs': list_all(limit)}
