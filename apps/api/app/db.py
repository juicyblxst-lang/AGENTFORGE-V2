...
def init_db():
    ...
    with conn() as c:
        c.execute(ddl)
        if DATABASE_URL:
            for col in ('budget_tx','register_tx','approval_tx','fund_tx','submit_tx','settle_tx','create_tx'): c.execute(f'ALTER TABLE executions ADD COLUMN IF NOT EXISTS {col} TEXT')
        else:
            ...
            if 'approval_tx' not in cols: c.execute('ALTER TABLE executions ADD COLUMN approval_tx TEXT')
